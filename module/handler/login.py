...
from module.base.utils import color_similarity_2d, crop, random_rectangle_point, random_normal_distribution_int
...
class LoginHandler(UI):
    ...
    def handle_4399_floating_ball(self):
        ...
        Returns:
            bool: If handled.
        """
        sim, floating_ball = TEMPLATE_LOGIN_4399_FLOATING_BALL_L.match_result(
            image = self.device.image,
            name = 'LOGIN_4399_FLOATING_BALL_L')
        if sim > 0.85:
            p1 = random_rectangle_point(floating_ball.area)
            p2 = random_rectangle_point((400, 570, 880, 650))
            self.device.swipe(p1, p2)
            return True
        sim, floating_ball = TEMPLATE_LOGIN_4399_FLOATING_BALL_R.match_result(
            image = self.device.image,
            name = 'LOGIN_4399_FLOATING_BALL_R')
        if sim > 0.85:
            p1 = random_rectangle_point(floating_ball.area)
            p2 = random_rectangle_point((400, 570, 880, 650))
            self.device.swipe(p1, p2)
            return True
        sim, floating_ball = TEMPLATE_LOGIN_4399_FLOATING_BALL_T.match_result(
            image = self.device.image,
            name = 'LOGIN_4399_FLOATING_BALL_T')
        if sim > 0.85:
            p1 = random_rectangle_point(floating_ball.area)
            p2 = random_rectangle_point((400, 570, 880, 650))
            self.device.swipe(p1, p2)
            return True
        sim, floating_ball = TEMPLATE_LOGIN_4399_FLOATING_BALL_B.match_result(
            image = self.device.image,
            name = 'LOGIN_4399_FLOATING_BALL_B')
        if sim > 0.85:
            p1 = random_rectangle_point(floating_ball.area)
            p2 = random_rectangle_point((400, 570, 880, 650))
            self.device.swipe(p1, p2)
